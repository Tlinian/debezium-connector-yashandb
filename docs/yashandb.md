...
- zookeeper、kafka、 kafka connect 已安装
...
1. 下载debezium-connector-yashandb-2.4.0.jar包
...
4. 下载debezium相关依赖
   ...
   2. 将上面下载的文件里面的debezium-connector-oracle-2.4.2.Final.jar替换成debezium-connector-yashandb-2.4.0.jar

5. 在kafka安装目录创建文件夹（plugins/debezium-connector-yashandb）

6. 将以下文件放入目录plugins/debezium-connector-yashandb中

   1. debezium-connector-yashandb-2.4.0.jar
   2. debezium相关依赖
   3. YashanDB jdbc依赖
   4. YashanDB YStream依赖

7. 将绝对路径（/kafkahome/plugin/debezium-connector-yashandb）添加到Kafka connect的配置文件的plugin.path

8. 重启Kafka connect 进程

9. 配置debezium YashanDB connector, 如下：
   ...
   
   {
       ...
       "config": {
           ...
           "database.url" : "jdbc:yasdb://192.168.8.203:2688/SYS", 
...
A: 填写的参数'database.ystream.server.name'对应的YStream server 处于非运行状态或者非启动状态，请先执行DBMS_YSTREAM_ADM.START( server_name IN VARCHAR(64) )启动该YStream服务。
...
4. 下载debezium相关依赖
   ...
   2. 将上面下载的文件里面的debezium-connector-oracle-2.4.2.Final.jar替换成debezium-connector-yashandb-2.4.2.Final.jar

5. 在kafka安装目录创建文件夹（plugins/debezium-connector-yashandb）

6. 将以下文件放入目录plugins/debezium-connector-yashandb中

   1. debezium-connector-yashandb-2.4.2.Final.jar
   2. debezium相关依赖
   3. YashanDB jdbc依赖
...
#### 7.3 Decimal数值同步到Kafka后，为什么序列化出来数据出错呢？
